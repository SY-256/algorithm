...
def chokudai_search_action_with_time_threshold(
    state: MazeState, beam_width: int, beam_depth: int, time_threshold: int
) -> int:
    """時間制限付きchokudaiサーチで行動を決定する"""
...
def test_ai_score(game_number: int):
    ...
    for i in range(game_number):
        ...
        while not state.is_done():
            action = chokudai_search_action_with_time_threshold(
                state,
                beam_width=1,  # ビーム幅
                beam_depth=END_TURN,  # ビームの深さ
                time_threshold=10,  # 制限時間(ms)
            )
